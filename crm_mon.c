...
send_custom_string(const char *name)
{
    ...
    sprintf(env_name, "CRM_notify_custom_%s", name);
    
    setenv(env_name, name, 1);
...
send_custom_trap_dump_msg(xmlNode * msg)
{
    ...
    if (pid == 0) {
        ...
        xmlDocSetRootElement(xmlDoc, msg);  
        sprintf(filename, "/root/msg_%d.xml", rand());
...
static int
send_node_state_trap(xmlNode * rsc_op)
{
    ...
    node = crm_element_value(rsc_op, XML_ATTR_UNAME);
    
    if (node == NULL) {
        node = ID(n);
    }
    if (node == NULL) {
        crm_err("No node detected");
        return 1;
    }
